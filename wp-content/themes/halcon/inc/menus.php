...
class walker_texas_ranger extends Walker_Nav_Menu {
    ...
    function __construct($css_class_prefix) {
        ...
        $this->item_css_class_suffixes = array(
            ...
            'current_menu_item'         => '__item--active',
            'has_children'              => '__item--has-children',
            'item'                      => '__item',  
            'link'                      => '__link',
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $item_classes =  array(
            ...
            'current_menu_item'     => in_array("current-menu-item",$item->classes) ? $prefix . $suffix['current_menu_item'] : '',
            'depth_class'           => $depth >=1 ? $prefix . $suffix['sub_menu_item'] . ' ' . $prefix . $suffix['sub_menu'] . '--' . $depth . '__item' : '',
            'has_children_class'    => in_array("menu-item-has-children",$item->classes) ? $prefix . $suffix['has_children'] : '' ,
        ...
        if($this->css_class_prefix === 'hero-menu'){
          $item_output .= ' title="'.apply_filters('the_title', $item->title, $item->ID).'"';
        }
        $item_output .= '>';
